...
Reporter.prototype.init = function () {
  ...
  return this._initOptions().then(function () {
    ...
    self.logger.info('Initializing jsreport@' + self.version + ' in ' + self.options.mode + ' mode' +
      ((self.options.loadConfig ? (' using configuration file: ' + (self._appliedConfigFile || 'none')) : '')))
    ...
    return ensureStarted().then(function () {
      ...
      return self._startReaper(self.options.tempDirectory)
    }).then(function () {
      return self._startReaper(self.options.tempAutoCleanupDirectory)
    }).then(function () {
...
Reporter.prototype._initOptions = function () {
  ...
  return Promise.resolve().then(function () {
    ...
    self.options.tempDirectory = self.options.tempDirectory || path.join(os.tmpdir(), 'jsreport')
    self.options.tempAutoCleanupDirectory = path.join(self.options.tempDirectory, 'autocleanup')
    self.options.tempCoreDirectory = path.join(self.options.tempDirectory, 'core')
    ...
    self.options.tasks.tempCoreDirectory = self.options.temptempCoreDirectoryDirectory
    self.options.tasks.tempAutoCleanupDirectory = self.options.tempAutoCleanupDirectory
    ...
    if (!fs.existsSync(self.options.tempDirectory)) {
      mkdirp.sync(self.options.tempDirectory)
    }

    if (!fs.existsSync(self.options.tempAutoCleanupDirectory)) {
      mkdirp.sync(self.options.tempAutoCleanupDirectory)
    }

    if (!fs.existsSync(self.options.tempCoreDirectory)) {
      mkdirp.sync(self.options.tempCoreDirectory)
    }
  })
}
...
Reporter.prototype._loadConfig = function () {
  ...
  var configFileParam = nfn.get('configFile')
  if (configFileParam) {
    var configFilePath = path.isAbsolute(configFileParam) ? configFileParam : path.join(this.options.rootDirectory, configFileParam)
  ...
  if (!this._appliedConfigFile) {
    var envBasedConfigFile = 'dev.config.json'
