...
function Persistence (options) {
  ...
  if (this.syncModifications) {
    ...
    this.watcher.on('all', function (eventName, filePath) {
      ...
      // ignore `addDir` events and OSX .DS_Store files
      if (eventName === 'addDir' || path.basename(filePath) === '.DS_Store') {
        return;
      }
...
Persistence.prototype.parseEntryDirectory = function (directory, cb) {
  ...
  fs.readFile(path.join(directory, 'config.json'), 'utf8', function (err, content) {
    ...
    // read the file based properties like content.handlebars
    fs.readdir(directory, function (err, files) {
      ...
      async.each(self.documentProperties, function (prop, cb) {
        // the document property definition doesn't include file extension, because this can be different (.html, .css, .handlebars)
        // we search for file by its name ignoring the file extension        
        var matchingFile = _.find(files, function (f) {
          var fileWithoutExtension = f.substring(0, f.lastIndexOf('.'))   
          // always using the last nested path, phantom.header is stored just as header.html       
          return fileWithoutExtension === _.last(prop.path.split('.'))
        })        
        ...
        fs.readFile(path.join(directory, matchingFile), function (err, content) {
          if (err) {
            return cb(err);
          }
